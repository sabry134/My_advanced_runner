...
void server(std::vector<int>& clientSockets, int port) {
    ...
    while (true) {
        ...
        if (FD_ISSET(serverSocket, &readfds)) {
            ...
            std::cout << "New client connected! Total players: " << clientSockets.size() << std::endl;
        }
        ...
        for (size_t i = 0; i < clientSockets.size(); i++) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                char buffer[1];
                int bytesRead = recv(sd, buffer, sizeof(buffer), MSG_PEEK);

                if (bytesRead <= 0) {
                    std::cout << "Player disconnected! Remaining players: " << clientSockets.size() - 1 << std::endl;
                    close(sd);
